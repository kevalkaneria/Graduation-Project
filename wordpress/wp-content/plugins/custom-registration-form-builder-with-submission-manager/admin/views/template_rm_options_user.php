...
jQuery(document).ready(function(){
        jQuery( "#options_users-element-1-0" ).next('label').after( '<span class="rm-option-subtext">Once user successfully submits a registration form, his/her account is created and activated automatically.</span>' );
        jQuery( "#options_users-element-1-1" ).next('label').after( '<span class="rm-option-subtext">Once user successfully submits a registration form, his/her account is created but in deactivated state. Useful for manually approving accounts via User Manager or admin notification email link. You can also activate accounts based on form properties using Automation.</span>' );
        jQuery( "#options_users-element-1-2" ).next('label').after( '<span class="rm-option-subtext">Once user successfully submits a registration form, he/she will receive an email with account verification link. Clicking the link will activate his/her account. Admin can also manually approve unverified accounts.<br>Please note, if you are using paid registration, the user will be auto activated upon successful payment.</span>' );
    });
</script>
<?php   